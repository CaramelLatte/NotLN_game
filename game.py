...
def main():
    ...
    class Bullet(Entity):
        def __init__(self, color, x, y, width, height, speed, hp, targetx,
                     targety, damage):
    ...
    class Enemy(Entity):
        def __init__(self, color, x, y, width, height, speed, hp, bullet_speed,
                     shot_damage, collide_damage, can_fire, exp, behavior):
            super().__init__(color, x, y, width, height, speed, hp)
            self.rect = pygame.Rect(x, y, width, height)
        ...
        def move(self):
            ...
            self.dist = math.hypot(self.x - player.rect.x,
            ...
            if self.behavior == "rush":
                ...
                if self.direction == "left":
                    flank_angle = math.atan2(player.rect.y - self.y,
                                             player.rect.x - self.x) + 6
    ...
    class Toast:
        def __init__(self, text, x, y) -> None:
            ...
            self.x = x
            self.y = y
            self.surface = stat_font.render(self.text, False, (236, 236, 236),
                                            (108, 132, 137))
        ...
        def move(self):
            if self.rectangle.midtop[1] < 0 and self.hover > 0:
                ...
                if self.hover <= 0:
                    self.rectangle.y -= 1
                    if self.rectangle.midbottom[1] <= (0 -
    ...
    def spawn_enemies():
        ...
        if enemy_spawn > 1 and enemy_spawn < (1 + (difficulty * 2)):
            ...
            if enemies_left <= 20:
                if enemy_type <= 20:
                    e = Enemy((255, 255, 255), enemy_spawnx, enemy_spawny, 15,
                              15, 1, 3, 5, 1, 1, True, 1, "snipe")
                elif enemy_type > 20 and enemy_type <= 50:
                    e = Enemy((255, 0, 0), enemy_spawnx, enemy_spawny, 15, 15,
                              3, 1, 5, 1, 1, False, 1, "rush")
                elif enemy_type > 50 and enemy_type <= 80:
                    e = Enemy((0, 0, 255), enemy_spawnx, enemy_spawny, 15, 15,
                              2, 2, 5, 1, 1, True, 1, "flank")
                elif enemy_type > 80:
                    e = Enemy((0, 0, 0), enemy_spawnx, enemy_spawny, 15, 15, 2,
                              5, 1, 1, 1, False, 1, "rush")
                enemies.append(e)
    ...
    def spawn_boss():
        ...
        e = Enemy((0, 128, 0), enemy_spawnx, enemy_spawny, 40, 40, 4, 50, 10,
                  2, 3, True, 5, "rush")
        e.boss = True
        enemies.append(e)
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if ui_4.collidepoint(event.pos):
                    if player.skill_points >= 3 and player.gun[
                            "spray"] == False:
        ...
        if game_active:
            ...
            hp_surface = stat_font.render(f"HP: {player.hp}/{player.max_hp}",
                                          False, (0, 0, 0))
            hp_rectangle = hp_surface.get_rect(midleft=(10, 640))
            level_surface = stat_font.render(f"Lvl: {player.level}", False,
                                             (0, 0, 0))
            level_rectangle = level_surface.get_rect(midleft=(10, 670))
            sp_surface = stat_font.render(f"SP: {player.skill_points}", False,
                                          (0, 0, 0))
            ...
            if player.potion_effect == "speed":
                player.rect.x += 2 * (keys[pygame.K_RIGHT] -
                                      keys[pygame.K_LEFT]) * player.speed
                player.rect.x += 2 * (keys[pygame.K_d] -
                                      keys[pygame.K_a]) * player.speed
            else:
                player.rect.x += (keys[pygame.K_RIGHT] -
                                  keys[pygame.K_LEFT]) * player.speed
                player.rect.x += (keys[pygame.K_d] -
                                  keys[pygame.K_a]) * player.speed
            if player.rect.x <= 0:
                player.rect.x = 0
            if player.rect.x >= 1004:
                player.rect.x = 1004
            player.rect.y += (keys[pygame.K_DOWN] -
                              keys[pygame.K_UP]) * player.speed
            player.rect.y += (keys[pygame.K_s] -
                              keys[pygame.K_w]) * player.speed
            if player.rect.y <= 0:
                player.rect.y = 0
            ...
            if pygame.mouse.get_pressed()[0] and player.shot_cd <= 0:
                ...
                b = Bullet((0, 0, 0), player.rect.centerx, player.rect.centery,
                           (5 * player.shot_size), (5 * player.shot_size), 10,
                           1, x, y, player.damage)
                if player.gun["pierce"] == True:
                    b.pierce += 1
                bullets.append(b)
                if player.gun["spray"] == True:
                    b1 = Bullet((0, 0, 0), player.rect.centerx,
                                player.rect.centery, (5 * player.shot_size),
                                (5 * player.shot_size), 10, 1, x + 30, y + 30,
                                player.damage)
                    b2 = Bullet((0, 0, 0), player.rect.centerx,
                                player.rect.centery, (5 * player.shot_size),
                                (5 * player.shot_size), 10, 1, x - 30, y - 30,
                                player.damage)
                    if player.gun["pierce"] == True:
            ...
            if player.skills["freeze"] == False:
                ...
                if boss_spawned == False and enemies_left >= 0:
                    spawn_enemies()
                elif boss_spawned == False and enemies_left <= 0 and len(
                        enemies) == 0:
                    spawn_boss()
                    boss_spawned = True

                    enemies_left += (20 + (2 * (difficulty - 1)))
            ...
            for b in reversed(range(len(bullets))):
                ...
                if bullets[b].x <= -50 or bullets[b].x >= 1074 or math.hypot(
                        bullets[b].x - bullets[b].startx,
                        bullets[b].y - bullets[b].starty) >= bullets[b].range:
                    del bullets[b]
                ...
                if player.skills["freeze"] == False:
                    ...
                    if e.boss == False:
                        ...
                        if shoot <= 1 and e.can_fire == True:
                            ...
                            b = Bullet((255, 165, 0), e.rect.centerx,
                                       e.rect.centery, 5, 5, e.bullet_speed, 1,
                                       player.rect.centerx,
                                       player.rect.centery, e.shot_damage)
                        ...
                        if shoot == 1:
                            ...
                            b = Bullet((255, 165, 0), e.rect.centerx,
                                       ...
                                       player.rect.centerx,
                                       player.rect.centery, e.shot_damage)
                            b1 = Bullet((255, 165, 0), e.rect.centerx,
                                        e.rect.centery, 5, 5, e.bullet_speed,
                                        1, player.rect.centerx + 30,
                                        player.rect.centery + 30,
                                        e.shot_damage)
                            b2 = Bullet((255, 165, 0), e.rect.centerx,
                                        e.rect.centery, 5, 5, e.bullet_speed,
                                        1, player.rect.centerx - 30,
                                        player.rect.centery - 30,
                                        e.shot_damage)
            ...
            while player.exp >= player.xp_to_level:
                ...
                toast = Toast(
                    f"Leveled up! You are level {player.level} with {player.skill_points} SP.",
                    1024, -50)
            ...
            text_surface = font.render(
                f"Level: {player.level}/{player.max_hp}, Skill Points: {player.skill_points}",
                False, (64, 64, 64)).convert()
            text_rectangle = text_surface.get_rect(center=(screen.get_width() /
                                                           2, 50))
            screen.blit(text_surface, text_rectangle)
            game_over_surface = font.render("Game over!", False, (255, 0, 0))
            game_over_rectangle = game_over_surface.get_rect(
                center=(screen.get_width() / 2, screen.get_height() / 2))
            restart_prompt_surface = font.render("Press space to restart",
                                                 False, (0, 0, 0))
            restart_prompt_rectangle = restart_prompt_surface.get_rect(
